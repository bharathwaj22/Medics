...
function Enterprocess_update_form() {
  ...
  const validateFields = () => {
    ...
    // Check if the specificOption field is filled for each entry
    if (!formData.specificOption || formData.specificOption.trim() === "") {
      isValid = false;
    }
    ...
    // Check each field in the dynamic fields
    fields.forEach((field) => {
      if (!field.specificOption || field.specificOption.trim() === "") {
        isValid = false;
      }
      if (!field.fileUploaded) {
        isValid = false;
      }
    });
